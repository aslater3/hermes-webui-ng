...
const wsAuth = new WsAuthClient(dashboard, (signal) => connection.verifyAdmission(signal));
const gateway = new GatewayClient(wsAuth, { socketFactory: browser.socketFactory, heartbeatMs: 0, diagnostics: ring });
const connection: ConnectionStore = new ConnectionStore(dashboard, gateway, ring);
...
try {
  ...
  assert.equal(connection.state.rest, 'healthy'); assert.equal(connection.state.auth, 'auth-required'); assert.equal(gateway.state.phase, 'auth-required');
  ...
  assert.equal(gateway.state.phase, 'ready'); assert.equal(connection.state.auth, 'signed-in');
  ...
  const replay = new WebSocket(ticket.url, ticket.protocols, { origin }); sockets.push(replay); replay.on('error', () => {});
  const denied = await new Promise<number>((resolve, reject) => {
    ...
    replay.once('unexpected-response', (request, response) => { clearTimeout(timer); resolve(response.statusCode ?? 0); response.resume(); request.destroy(); });
    replay.once('open', () => { clearTimeout(timer); reject(new Error('Consumed ticket admitted twice')); });
  });
  assert.ok([401, 403].includes(denied), 'Hermes must reject a consumed ticket'); first.close(); passed('vanilla-one-use-ticket-replay-rejected');
  ...
  assert.ok(connection.report().events.length <= 500); passed('allowlisted-support-report');
  let cleared = false; connection.onIdentityBoundary(() => { cleared = true; });
  await dashboard.logout(); await connection.resume(); assert.ok(cleared);
  assert.equal(connection.state.auth, 'auth-required'); assert.equal(gateway.state.phase, 'auth-required');
  const mints = ring.snapshot().filter((entry) => entry.event === 'auth.ticket').length;
  await connection.resume(); assert.equal(ring.snapshot().filter((entry) => entry.event === 'auth.ticket').length, mints);
  passed('revoked-cookie-closes-existing-admission');
  await connection.login(provider.name, username, password); assert.equal(gateway.state.phase, 'ready');
  await connection.logout(); assert.equal(connection.state.auth, 'signed-out'); assert.equal(gateway.state.phase, 'disconnected');
  await assert.rejects(new WsAuthClient(dashboard).credential(), (error: unknown) => error instanceof ClientError && error.kind === 'auth-required');
  passed('verified-logout-and-fresh-sign-in'); success = true;
} catch (error) {
  console.error(error instanceof ClientError ? `${error.kind}: ${error.message}` : error instanceof Error ? error.message : 'Phase 1 acceptance failed'); process.exitCode = 1;
} finally {
