...
export interface WriteAuditEvent {
  event: 'workspace.write'; requestId: string; operation: 'save'; status: number; durationMs: number;
...
export class WriteAudit {
  ...
  begin(cookie: string | undefined) {
    ...
    return (status: number, outcome: WriteAuditEvent['outcome'], root?: string, path?: string, bytes?: number) => {
      ...
      const event: WriteAuditEvent = { event: 'workspace.write', requestId, operation: 'save', status,
