...
## Test counter correction

Run `34891230005` on `07edb24272fad2acdb710a3057545869abbef80f` passed **407/408** cases. All original cases passed, and the new iPhone case successfully rendered the returned catalogue with frames withheld. Its final instrumentation check failed because it counted *native frame callbacks that fired while paused*, but WebKit had not fired the pending native frame at all. The counter now measures the non-empty queue of withheld callbacks directly. It still requires the catalogue to render while a requested frame is undelivered; no application assertion or timeout was removed or weakened. This change is test/documentation only; application source stays at the directly subscribed implementation. Complete final CI is still required.
