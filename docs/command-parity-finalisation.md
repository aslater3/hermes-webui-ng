...
## Native interactive-request compatibility

Inspected the current public peer protocol at `NousResearch/hermes-agent@fef0bc56b2f622ffe124835fbf57adfd10aa17e6`. The Gateway now distinguishes server requests from client RPC replies, renders approval/clarify/sudo/secret/vault forms, and replies with the exact native request ID. Cancellation, duplicate frames, early batch locks and `open_requests` snapshot recovery are covered. Unknown desktop/controller surfaces fail fast with JSON-RPC -32601 rather than disconnecting or claiming they ran. Secret answers are not retained in a response queue, diagnostics or browser storage; stale/hidden questions cannot be answered. Ordinary sends and software updates are blocked while a current native question is awaiting input.

The older notification/respond contract remains unchanged for the pinned runtime. Unit and socket tests plus three four-project browser cases cover the new peer wire shapes. This is compatibility implementation based on current upstream contracts, not runtime certification of arbitrary current main. Full build/typechecks/lint, 313 unit cases and 34 wire contracts passed locally before publication; peer browser CI remains required.

## Browser-native attachment and session actions

The final browser-equivalent slice removes the remaining detached-worker mismatches for commands whose effect belongs to the selected GUI conversation. `/branch` now uses `session.branch` and opens the returned durable child; `/yolo` uses the existing session-scoped YOLO control; `/image` can attach an explicit Hermes-host path or upload an image from the browser as bytes; `/paste` reads an image from the browser clipboard only after an explicit user gesture and sends it through `image.attach_bytes`. Bare `/save` uses `session.save`; format/filename forms retain Hermes' native parser. Custom commands that shadow these names still retain native command ownership.

Image bytes are bounded to 25 MiB, never written to browser storage, and remain queued only in the selected native Hermes session for the next prompt. Session/profile/account/connection changes fail closed; an attachment that may already have reached Hermes is never replayed automatically. Terminal-only display controls with no meaningful WebUI analogue remain surface-specific rather than being reported as successful browser operations.
