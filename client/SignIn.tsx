...
export function SignIn({ runtime: rt, onSettings }: { runtime: AppRuntime; onSettings: () => void }) {
  ...
  if (state.offline) return <div className="signin-layout"><div className="signin-top"><Brand/><button className="text-button" onClick={onSettings}>App settings</button></div><main className="signin-main"><div className="signin-card"><h1>You’re offline.</h1><p className="signin-intro">The app shell is available, but conversations are not cached. Reconnect to Hermes to restore your session. No message will be replayed.</p><button className="primary" onClick={() => rt.run(async () => { if (navigator.onLine) { await rt.connection.setOffline(false); await rt.connection.start(); } })}>Retry connection</button></div></main></div>;
  if (rt.dashboard.admissionMode === 'trusted-local') return <div className="signin-layout"><div className="signin-top"><Brand/><button className="text-button" onClick={onSettings}><Settings2 size={16}/>Connection settings</button></div><main className="signin-main"><div className="signin-card"><span className="eyebrow">TRUSTED LAN · NO LOGIN</span><h1>Your local agent<br/>is unavailable.</h1><p className="signin-intro">Check that Hermes is running and the server-side local token is valid. No password sign-in is configured here.</p>{error && <Notice error>{error}</Notice>}<button className="primary signin-submit" disabled={state.busy || state.offline} onClick={() => rt.run(() => rt.connection.start())}>Retry local connection<ArrowRight size={18}/></button></div></main></div>;
