...
export default function App({ runtime: rt }: { runtime: AppRuntime }) {
  ...
  useEffect(() => { if (!hasAccess && panel !== 'settings') setPanel(null); setCopyNote(''); }, [rt.accountGeneration]);
  ...
  return <>
    {!hasAccess ? <SignIn key={rt.accountGeneration} runtime={rt} onSettings={() => setPanel('settings')}/> : <div className={`workspace${collapsed ? ' rail-collapsed' : ''}`}>
