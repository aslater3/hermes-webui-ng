...
export function useCommands(rt: AppRuntime, draft: string, setDraft: (text: string) => void,
  ...
  useEffect(() => { setPanel(null); setQuery(''); setDismissed(undefined); },
  ...
  const open = () => { trigger.current?.focus({ preventScroll: true }); setQuery(''); setPanel('catalogue'); };
  const close = () => {
  ...
  const popup = suggesting ? <div className="command-suggestions">
    ...
    {state.catalogue && <p className="command-match-count" role="status">{matches.length} matching · {matches.filter(enabled).length} available in WebUI</p>}
  ...
  const overlay = eligible && (panel || state.result) ? createPortal(<Modal title={state.result ? `Command ${state.result.command}` : panel === 'context' ? 'Usage & context' : 'Hermes commands'} kind="commands" onClose={close}>
    <div className="commands-body">
      {state.result ? <>
        ...
        <p>Commands advertised by the selected Hermes profile. “Not implemented” is a WebUI feature gap, not a login or permission error.</p>
        ...
        {state.catalogue && <p className="command-match-count" role="status">{rows.length} matching · {rows.filter(enabled).length} available in WebUI</p>}
