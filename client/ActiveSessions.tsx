...
export function ActiveSessions({ runtime: rt, onChoose }: { runtime: AppRuntime; onChoose: () => void }) {
  ...
  return <section className="active-sessions" aria-label="Active agent sessions">
    ...
    <ul>{items.slice(0, expanded ? 100 : 6).map(item => <li key={item.runtimeId}><button type="button" className="active-session-row" aria-label={`Open active session: ${item.title || 'Native conversation'}`} disabled={!rt.ready || rt.chat.busy} onClick={() => { rt.openLive(item.runtimeId); onChoose(); }}>
      {item.status === 'waiting' ? <CircleAlert size={16} aria-hidden="true"/> : ['working', 'starting'].includes(item.status) ? <LoaderCircle size={16} aria-hidden="true"/> : <Check size={16} aria-hidden="true"/>}
      <span><strong>{item.title || 'Native conversation'}</strong><small>{item.status === 'waiting' ? 'Needs your input' : item.status === 'working' ? 'Working…' : item.status === 'starting' ? 'Starting…' : item.status === 'unknown' ? 'Reconnect to check status' : 'New activity'}</small></span>
    </button></li>)}</ul>
